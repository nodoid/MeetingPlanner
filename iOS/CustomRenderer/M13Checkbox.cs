        ...
        new void LayoutSubviews()
        {
            ...
            if (CheckAlignment == CheckboxAlignment.Right)
            {
                CheckView.Frame = new CGRect(
                    ...
                    (Frame.Size.Height - ((Constants.BoxSize + Constants.CheckHorizontalExtention) * heightForCheckbox)) / 2.0,
                    ((Constants.BoxSize + Constants.CheckHorizontalExtention) * heightForCheckbox),
                    heightForCheckbox)
                    .Integral();
                ...
                TitleLabel.Frame = new CGRect(
                    0,
                    0,
                    Frame.Size.Width - (heightForCheckbox * (Constants.BoxSize + Constants.CheckHorizontalExtention + Constants.CheckBoxSpacing)),
                    Frame.Size.Height)
                    .Integral();
            }
            else {
