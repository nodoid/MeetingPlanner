...
namespace MeetingPlanner
{
    internal class EncryptionManager : IEncryptionManager
    {
        ...
        Encryption IEncryptionManager.EncryptUsers(IEnumerable<ActiveDirectoryUser> users)
        {
            string usersStr = JsonConvert.SerializeObject(users);

            var key = _cryptographyService.GenerateRandomNumber(32);
            var iv = _cryptographyService.GenerateRandomNumber(16);

            var encryption = _cryptographyService.AesEncrypt(Encoding.UTF8.GetBytes(usersStr), key, iv);

            var encryptionHashmac = _cryptographyService.ComputeHashSha256(encryption, key);

            var rsakey = _cryptographyService.RSAEncrypt(key, ServerPublicKey);

            string encryptionStr = Convert.ToBase64String(encryption);
            string encodedKey = Convert.ToBase64String(rsakey);
            string encodedIv = Convert.ToBase64String(iv);
            string encodedHashmac = Convert.ToBase64String(encryptionHashmac);

            Encryption encryptionObj = new Encryption() { Category = "UserData", Value = encryptionStr, Key = encodedKey, IV = encodedIv, Hashmac = encodedHashmac };

            return encryptionObj;
        }


        IEnumerable<ActiveDirectoryUser> IEncryptionManager.DecryptUsers(IEnumerable<Encryption> encryptions)
        {
            IList<ActiveDirectoryUser> users = new List<ActiveDirectoryUser>();

            foreach (Encryption encryption in encryptions)
            {
                if (encryption.Category == "UserData")
                {
                    var dataToDecrypt = Convert.FromBase64String(encryption.Value);
                    var rsaEncryptedkey = Convert.FromBase64String(encryption.Key);
                    var iv = Convert.FromBase64String(encryption.IV);
                    ...
                    var userObjects = JsonConvert.DeserializeObject<IEnumerable<ActiveDirectoryUser>>(decryptedString);

                    foreach (var usr in userObjects)
                    {
                        users.Add(usr);
                    }
                }
        ...
        bool IEncryptionManager.Compare(byte[] array1, byte[] array2)
        {
