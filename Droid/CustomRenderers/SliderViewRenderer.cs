...
namespace MeetingPlanner.Droid
{
    ...
    {
        readonly GesutreListener listener;
        ...
        protected override void Dispose(bool disposing)
        {
            if (sliderView.CurrentView is Layout)
            {
                ...
                var currentLayout = (ViewGroup)view.GetChildAt(0);
                    ...
                    if (Math.Abs(delta) > sliderView.MinimumSwipeDistance)
                    {
                        if (delta > 0)
                        {
                            if (currentViewIndex != 0)
                            {
                                ...
                                var loading = await TranslateToCurrentViewAsync("Right");
                        ...
                        else if (delta < 0)
                        {
                            if (sliderView.Children.Count > currentViewIndex + 1)
                            {
                                currentViewIndex++;
                                sliderView.CurrentView = sliderView.Children[currentViewIndex];
                                var loading = await TranslateToCurrentViewAsync("Left");
        ...
        public async Task<bool> TranslateToCurrentViewAsync(string direction)
        {
            ...
            {
                var view = (ViewGroup)this.ViewGroup.GetChildAt(0);
                var currentLayout = (ViewGroup)view.GetChildAt(1);
                currentLayout.Touch += HandleGenericMotion;
            }
            return error;
        }

        protected override AndroidView CreateNativeControl()
        {
            return new AndroidView(Forms.Context);
        }
    }
}
